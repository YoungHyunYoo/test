...
void DirectionSelector::ProcessAreaCodesMap(char *line, int ln)
{
	...
	if(!side || !areaCode) // no char type's data or no :
	{
...
//printf("__Direction Selector start\n");
...
//printf("1\n");
			//Iterating through area codes map to check which side will be retain
				...
				for(it = pDirectionSelector->m_areaCodesMap.begin(); it!= pDirectionSelector->m_areaCodesMap.end() && found == false; it++) // __ not run
				{
//printf("2\n");
...
//printf("__Direction selector end\n");
			}
		}
