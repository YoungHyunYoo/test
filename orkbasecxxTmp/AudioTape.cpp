...
{
printf("1 AudioTape::AddAudioChunk : %d\n",chunkRef.use_count());
//printf("___AudioTape::AddAudioChunk start\n");
	...
	if(m_state == StateCreated || m_state == StateActive)
	{
		...
		if(CONFIG.m_sttEnable==true)
		{
//			AudioChunkRef sttChunkRef;
//			sttChunkRef = chunkRef;
//			m_sttChunkQueue.push(sttChunkRef);
			m_sttChunkQueue.push(chunkRef);
//			m_sttChunkQueueDataSize += sttChunkRef->GetNumBytes();
...
printf("2 AudioTape::AddAudioChunk : %d\n",chunkRef.use_count());



		m_chunkQueue.push(chunkRef);
printf("3 AudioTape::AddAudioChunk : %d\n",chunkRef.use_count());
...
void AudioTape::WriteStt()
{
        ...
        if(m_sttState == StateCreated && PartyFilterActive()) // __ not run
        {
        ...
        while(!done && m_sttState != StateStopped && m_sttState != StateError) 
        {
                ...
                AudioChunkRef sttChunkRef;
                {
                        ...
                        if (m_sttChunkQueue.size() > 0) 
                        {
                                sttChunkRef = m_sttChunkQueue.front();
printf("1 WriteStt : %d\n",sttChunkRef.use_count());
                                m_sttChunkQueue.pop();
...
printf("4 WriteStt : %d\n",sttChunkRef.use_count());
                                ...
                                {
					m_audioSttFileRef->WriteChunk(sttChunkRef);
printf("5 WriteStt : %d\n",sttChunkRef.use_count());
					m_sttBytesWritten+=sttChunkRef->GetNumBytes();
                                }
                        }
                        catch (CStdString& e)
                        {
                                ...
                                if(m_numSttErrors <= 3)
                                {
                                        logMsg.Format("[%s] sttBytesWritten:%d %s", m_trackingId, m_sttBytesWritten, e);
...
/*			if(m_sttBytesWritten>0)
			{
printf("\n\nsttReadyForBatchProcessing = true\n");
...
//printf("m_fileExtension : %s\n",m_fileExtension.c_str());
				...
				{
//printf("5\n");
//printf("%d\n",CONFIG.m_captureFileSizeLimitKb);
...
{
//printf("___AudioTape::AddCaptureEvent Start\n");
        CStdString logMsg;
...
{
//printf("TapeAttributeToEnum : %s\n",ta.c_str());
//printf("compareNocase : %d\n",ta.CompareNoCase(TA_TRACKINGID));
