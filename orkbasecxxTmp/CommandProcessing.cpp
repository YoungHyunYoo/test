...
//printf("__CommandProcessing start\n");
			if(audioTapeRef.get() == NULL) // does not exist audioTapeRef
			{
			...
			else // exist audioTapeRef , loop scope until Line 256
			{
				CStdString command = CONFIG.m_commandProcessingCommand;
				trackingId = audioTapeRef->m_trackingId;

				if(command.length() == 0) // doesen't exist command line. Read unter Error message
				{
				...
				if(!audioTapeRef->m_isExternal) // __ run here
				{
//printf("1\n");
					audioFileName = audioTapeRef->m_audioOutputPath + "/" + audioTapeRef->GetPath() + audioTapeRef->GetIdentifier() + ".wav" ;
					//as usual
				}
				else // file location is external
				{
//printf("2\n");
					audioFileName = audioTapeRef->m_audioOutputPath + "/" + audioTapeRef->GetPath() + audioTapeRef->m_externalFileName;
					//conventional orkaudio name + orginal name (include extension)
				}
...
#ifdef WIN32
				if( (int)_spawnl(_P_WAIT, command ,command + " " + audioFileName, NULL) < 0) // WIN32
				{
				...
				int pos_arg = 0;
				if(pos_arg = command.Find(inArg) == -1) // doesn't exitst "[IN]"  // __ run here
				{
//printf("3\n");
					system(command);
				}
