...
{
//printf("__ Im TapeFileNaming AdduaudioTape\n");
...
void TapeFileNaming::ThreadHandler(void *args)
{
	...
	for(;stop == false;)
	{
		...
		try
		{
//printf("\n__ before TapeFileNaming pop\n");
			audioTapeRef = pTapeFileNaming->m_audioTapeQueue.pop();
//printf("__after TapeFilenaming pop\n");
...
//printf("\n\n%s\n%s\n",originalFilename.c_str(),newFilename.c_str());
				if(originalFilename.Compare(newFilename) != 0) // __ not run
				{
//printf("1\n");
					if(ACE_OS::rename((PCSTR)originalFilename, (PCSTR)newFilename) != 0) //__ not run
					{
//printf("2\n");
