...
{
	if(m_sipDialedNumber.length() != 0) // __ not run
	{
//printf("1\n");
	...
	if(DLLCONFIG.m_sipDirectionReferenceIpAddresses.m_asciiIpRanges.size()) // __ not run
	{
//printf("2\n");
...
//printf("5\n");
		...
		if(MatchesSipDomain(m_invite->m_fromDomain) && MatchesSipDomain(m_invite->m_toDomain)) // __ not run
		{
//printf("6\n");
			// Both match at least one entry
			ProcessMetadataSipOutgoing();
		}
		else if(MatchesSipDomain(m_invite->m_fromDomain)) // __ not run
		{
//printf("7\n");
			// Only from domain matches
			ProcessMetadataSipOutgoing();
		}
		else if(MatchesSipDomain(m_invite->m_toDomain)) // __ not run
		{
//printf("8\n");
			// Only to domain matches
			ProcessMetadataSipIncoming();
		}
		else if(MatchesStringList(m_invite->m_userAgent, DLLCONFIG.m_sipDirectionReferenceUserAgents)) // __ not run
		{
//printf("9\n");
			ProcessMetadataSipIncoming();
		}
...
{
//printf("___ReportMetadata start\n");
	CStdString logMsg;
	char szLocalIp[16];
	ACE_OS::inet_ntop(AF_INET, (void*)&m_localIp, szLocalIp, sizeof(szLocalIp));
	char szRemoteIp[16];
	...
	if(DLLCONFIG.m_localPartyForceLocalIp) // __ not run
	{
//printf("___ReportMetadata 1\n");
		CStdString lp(szLocalIp);
		m_localParty = VoIpSessionsSingleton::instance()->GetLocalPartyMap(lp);
	}
	// Check if we don't have the local party based on the endpoint IP address
	else if(m_localParty.IsEmpty()) // __ not run
	{
//printf("___ReportMetadata 2\n");
...
//printf("___ReportMetadata 3\n");
	...
	{
//printf("___ReportMetadata 4\n");
		if(DLLCONFIG.m_useMacIfNoLocalParty)
		{
//printf("___ReportMetadata 5\n");
			MemMacToHumanReadable((unsigned char*)m_localMac, m_localParty);
		}
		else
		{
//printf("___ReportMetadata 6\n");
			CStdString lp(szLocalIp);
			m_localParty = VoIpSessionsSingleton::instance()->GetLocalPartyMap(lp);
		}
	}
...
//printf("___ReportMetadata 7\n");
	...
	if(m_isCallPickUp)
	{
//printf("___ReportMetadata 8\n");
		if(m_direction == CaptureEvent::DirIn)
		{
//printf("___ReportMetadata 9\n");
			m_direction = CaptureEvent::DirOut;
		}
		else
		{
//printf("___ReportMetadata 10\n");
			m_direction = CaptureEvent::DirIn;
		}
...
//printf("___ReportMetadata 13\n");
	...
	{
//printf("___ReportMetadata 14\n");
		if(DLLCONFIG.m_localPartyAddLocalIp == true)
		{
//printf("___ReportMetadata 15\n");
			char szLocalIp[16];
			ACE_OS::inet_ntop(AF_INET, (void*)&m_localIp, szLocalIp, sizeof(szLocalIp));
			m_localPartyName.Format("%s@%s", m_localPartyName, szLocalIp);
		}
	...
	{
//printf("___ReportMetadata 16\n");
		if(DLLCONFIG.m_localPartyAddLocalIp == true)
		{
//printf("___ReportMetadata 17\n");
			char szLocalIp[16];
			ACE_OS::inet_ntop(AF_INET, (void*)&m_localIp, szLocalIp, sizeof(szLocalIp));
			m_localParty.Format("%s@%s", m_localParty, szLocalIp);
		}
	...
	{
//printf("___ReportMetadata 18\n");
		event->m_value = m_remotePartyName;
	}
	else
	{
//printf("___ReportMetadata 19\n");
		event->m_value = m_remoteParty;
	}
	g_captureEventCallBack(event, m_capturePort);
...
{
//printf("\n__Im AddRtpPacket start\n");
	CStdString logMsg;
	unsigned char channel = 0;
	if(DLLCONFIG.m_rtpLogAllSsrc == true) // __ not run
	{
...
{
//printf("_____ReportSipInvite start\n");
	if(DLLCONFIG.m_sipMetadataUseLastInvite || m_invite.get() == NULL) // __ first INVITE, run here
	{
//printf("_____ReportSipInvite 1\n");
		m_invite = invite;
	}
	else // __ second, third INVITE, run here
	{
//printf("_____ReportSipInvite 2\n");
		CStdString inviteString;
		CStdString logMsg;
		invite->ToString(inviteString);
	...
	if(invite->m_from.CompareNoCase(invite->m_to) == 0 && invite->m_contact.length() > 0) // __ not run
	{
//printf("_____ReportSipInvite 3\n");
	...
	if(invite->m_telephoneEventPtDefined) // __ not run
	{
//printf("_____ReportSipInvite 4\n");
	...
	if((invite->m_sipMethod == SIP_METHOD_INVITE) && (invite->m_sipRemoteParty != "")) // __ not run
	{
//printf("_____ReportSipInvite 5\n");
...
//printf("_____ReportSipInvite 7.1\n");
	...
	{
//printf("_____ReportSipInvite 9\n");
		std::copy(invite->m_extractedFields.begin(), invite->m_extractedFields.end(), std::inserter(m_tags, m_tags.begin()));
	}

	if(DLLCONFIG.m_sipOnDemandFieldName.size()) // first,second,third INVITE, run here
	{
//printf("_____ReportSipInvite 10\n");
		std::map<CStdString, CStdString>::iterator pair;

		pair = m_tags.find(DLLCONFIG.m_sipOnDemandFieldName);
		if(pair != m_tags.end()) // __ not run
		{
//printf("_____ReportSipInvite 11\n");
			CStdString value;
...
//printf("_____end Report Sip Invite\n");
}
...
{
//printf("s::reportSipInvite Start\n");
	if(DLLCONFIG.m_sipIgnoredMediaAddresses.Matches(invite->m_fromRtpIp)) // __ not run 
	{
//printf("s::reportSipInvite 1\n");
...
//printf("szRtpPort : %s\n",szRtpPort);
	...
	if(*errorLocation != '\0') // __ not run
	{

//printf("s::reportSipInvite 2\n");
		rtpPortAsInt = 0;
	}

	unsigned short rtpPort = 0;
	if(rtpPortAsInt>0 && rtpPortAsInt<65535) // __ first,second,third INVITE, run here
	{

//printf("s::reportSipInvite 3\n");
	...
	if(session.get() == NULL && DLLCONFIG.m_sipTrackMediaAddressOnSender) // __ not run
	{

//printf("s::reportSipInvite 4\n");
	...
	if(session.get()) // __ second INVITE, run here
	{

//printf("s::reportSipInvite 5\n");
		// A session already exists on this media address
		if(session->m_protocol == VoIpSession::ProtRawRtp) // __ not run
		{
//printf("s::reportSipInvite 5.1\n");
			// Do nothing here so that we end up stopping this Raw RTP session 
			// and creating new session below
		}
	    else if(session->m_protocol == VoIpSession::ProtSip && DLLCONFIG.m_rtpAllowMultipleMappings == true) // __ second INVITE, run here
	    {
//printf("s::reportSipInvite 5.2\n");
	    	// Do nothing here so that this new session ends up receiving
			// all future RTP packets to this media address
	    }
		else if (DLLCONFIG.m_rtpAllowMultipleMappings == false && invite->m_callId.Equals(session->m_callId) == false) // __ not run
		{
//printf("s::reportSipInvite 5.3\n");
		...
		{
//printf("s::reportSipInvite 5.4\n");
			if(invite->m_attrSendonly) // __ not run
			{
//printf("s::reportSipInvite 5.5\n");
...
//printf("s::reportSipInvite 5.6\n");
			...
			if(DLLCONFIG.m_sipCallPickUpSupport) // __ not run
			{
//printf("s::reportSipInvite 5.7\n");
	...
	if (pair != m_byCallId.end()) // __ second, third INVITE, run here
	{
//printf("s::reportSipInvite 6\n");
		...
		if(invite->m_attrSendonly) // __ not run
		{
//printf("s::reportSipInvite 6.1\n");
			session->GoOnHold(invite->m_recvTime);
			LOG4CXX_INFO(m_log, "[" + session->m_trackingId + "] SIP session going on hold");
			return;
		}
		else // __ second, third INVITE, run here // __ sendrecv??
		{
//printf("s::reportSipInvite 6.2\n");
			/* If we're already on hold and sendonly is not present
			 * then we go off hold */
			if(session->m_onHold && DLLCONFIG.m_sipInviteCanPutOffHold) // __ not run
			{
//printf("s::reportSipInvite 6.3\n");
				session->GoOffHold(invite->m_recvTime);
				session->m_lastUpdated = time(NULL);	// so that timeout countdown is reset
				LOG4CXX_INFO(m_log, "[" + session->m_trackingId + "] SIP session going off hold");
				SetMediaAddress(session, invite->m_fromRtpIp, rtpPort);
...
//printf("s::reportSipInvite 6.3.1\n");
		if(session->m_ipAndPort != ipAndPort && DLLCONFIG.m_sipDynamicMediaAddress) // third INVITE, run here
		{
//printf("s::reportSipInvite 6.4\n");
			SetMediaAddress(session, invite->m_fromRtpIp, rtpPort);
			if(DLLCONFIG.m_sipTrackMediaAddressOnSender) // __ not run
			{
//printf("s::reportSipInvite 6.5\n");
				SetMediaAddress(session, invite->m_originalSenderIp, rtpPort);
			}
		}
		if(DLLCONFIG.m_sipCallPickUpSupport)//__ not run
		{
//printf("s::reportSipInvite 6.6\n");
			TrySessionCallPickUp(invite->m_replacesId, session->m_isCallPickUp);
		}
//printf("s::reportSipInvite 6.6.0\n");
		session->ReportSipInvite(invite); // __ run second, third INVITE
		return;
	}
//printf("s::reportSipInvite 6.6.1\n");
...
//printf("s::reportSipInvite 7\n");
	...
	if (DLLCONFIG.m_sipNotifySupport) { // __ not run
//printf("s::reportSipInvite 8\n");
...
{
//printf("___ SetMediaAddress start :");
	if(mediaPort == 0) // __ not run
	{
//printf("1 ");
		return;
	}
	if(DLLCONFIG.m_mediaAddressBlockedIpRanges.Matches(mediaIp)) // __ not run
	{
//printf("2 ");
		char szMediaIp[16];
		CStdString logMsg;
	...
	if(!DLLCONFIG.m_mediaAddressAllowedIpRanges.Empty()) // __ not run
	{
//printf("3 ");
...
//printf("10 ");
	...
	if(doChangeMediaAddress) // __ run here
	{
//printf("11 ");
		if(m_log->isInfoEnabled()) // __ run here
		{
//printf("12 ");
			char szEndPointIp[16];
			ACE_OS::inet_ntop(AF_INET, (void*)&session->m_endPointIp, szEndPointIp, sizeof(szEndPointIp));
		...
		if (DLLCONFIG.m_rtpAllowMultipleMappings == false) // __ not run
		{
//printf("13 ");
...
printf("ReportSkinnyStopMediaTransmission\n\n");
			Stop(session);
		}
	}
	else
...
void VoIpSessions::ReportSkinnySoftKeySetTransfConnected(SkSoftKeySetDescriptionStruct* skEvent, IpHeaderStruct* ipHeader, TcpHeaderStruct* tcpHeader)
{
	...
	if(session.get())
	{
		...
		LOG4CXX_INFO(m_log, logMsg);
printf("ReportSkinnySoftKeySetRransfConnected\n\n");
		Stop(session);
	}
}
...
{
//printf("__ReportRtpPacket start\n");
	...
	if(DLLCONFIG.m_sangomaEnable && sourcePort == destPort) // __ not run
	{
//printf("1 ");
	...
	if (session1.get() != NULL) // __ run here when calling
	{
//printf("\n2\n");
//printf("_____session1 is not null\n");
...
//printf("\n");
		...
		{
//printf("\n_____session1->addRtpPacket sucess\n");
			numSessionsFound++;
		}
		else
		{
//printf("\n_____session1->addRtpPacket fail\n");
			// RTP discontinuity detected
printf("ReportRtpPacket1\n\n");
			Stop(session1);
		}
	}
//printf("_____\nsession1 is end\n");
	...
	if (session2.get() != NULL) // __ run here when not print
	{
//printf("_____session2 is not null\n");
//printf("\n3\n");
		// Found a session give it the RTP packet info
		session = session2;
//printf("\n");
		if(session2->AddRtpPacket(rtpPacket)) // __ run here
		{

//printf("\n_____session2->addRtpPacket sucess\n");
			numSessionsFound++;
		}
		else // __ not run
		{
...
//printf("\n_____session2->addRtpPacket fail\n");
			// RTP discontinuity detected
printf("ReportRtpPcaket2\n\n");
			Stop(session2);
		}
	}
//else printf("_____session2 is null\n");
	if(numSessionsFound == 2 && session1.get() != session2.get()) // __ not run
	{
//printf("4 ");
		...
		else
		{
			...
			if(DLLCONFIG.m_rtpAllowMultipleMappings == true && session1->m_protocol == VoIpSession::ProtSip && session2->m_protocol == VoIpSession::ProtSip)
			{
				if(session1->m_sipLastInvite > session2->m_sipLastInvite)
				{
					...
					{
printf("takes over media address1\n\n");
						unsigned long long ipAndPort;
				...
				else
				{
					...
					{
printf("takes over media address2\n\n");
						unsigned long long ipAndPort;
...
printf("reportRtpPacket\n\n");
		Stop(mergeeSession);
	}
	else if(numSessionsFound == 1 ) // __ run here 
	{
//printf("5\n");
		if(DLLCONFIG.m_mediaAddressUseSecondRtpAddress == true)	//either source or dest media address is not already in tracking map // __ not run
		{
//printf("5.1\n");
...
//printf("5.3\n");
		...
		if (session->m_numRtpPackets == 1 && session->m_protocol == VoIpSession::ProtSip) // __ run here when call open
		{
//printf("5.4\n");
			// This was the first packet of the session, check whether it is tracked on the right IP address
			in_addr trackingIp;
			unsigned short trackingPort = 0;
			if(DLLCONFIG.IsRtpTrackingIpAddress(rtpPacket->m_destIp)) // __ not run
			{
//printf("5.5\n");
				trackingIp = rtpPacket->m_destIp;
				trackingPort = rtpPacket->m_destPort;
			}
			else if(DLLCONFIG.IsRtpTrackingIpAddress(rtpPacket->m_sourceIp)) // __ not run
			{
//printf("5.6\n");
				trackingIp = rtpPacket->m_sourceIp;
				trackingPort = rtpPacket->m_sourcePort;
			}
			if(trackingPort) //__ not run
			{
//printf("5.7\n");
				SetMediaAddress(session, trackingIp, trackingPort);
			}
	...
	else if((numSessionsFound == 0) && ((CONFIG.m_lookBackRecording == true) || DLLCONFIG.m_trackRawRtpSessionInNonLookBackMode == true)) // __ run here when call open and exit
	{
//printf("\n\n6\n");
		VoIpEndpointInfoRef endpoint;

		// Check if a Skinny session can be found on the endpoint
		if(DLLCONFIG.m_skinnyRtpSearchesForCallInfo) // __ not run
		{
//printf("6.1\n");
			if(DLLCONFIG.m_skinnyBehindNat != true)
			{
...
//printf("6.5\n");
		...
		if(DLLCONFIG.IsRtpTrackingIpAddress(rtpPacket->m_sourceIp)) // __ not run
		{
//printf("6.6\n");
			rtpIp = rtpPacket->m_sourceIp;
			rtpPort = rtpPacket->m_sourcePort;
		}
		else if(DLLCONFIG.m_sangomaEnable) // __ not run
		{
//printf("6.7\n");
			rtpIp = rtpPacket->m_sourceIp;
			rtpPort = rtpPacket->m_sourcePort;
...
//printf("6.8.6\n");
			...
			if(endpoint->m_ip.s_addr == rtpPacket->m_sourceIp.s_addr)// __ not run
			{
//printf("6.10\n");
...
CStdString VoIpSessions::StopCaptureNativeCallId(CStdString& nativecallid, CStdString& qos)
{
	...
	if(found)
	{
		...
		VoIpSessionRef newSession(new VoIpSession(nextTrackId));
		CopyMetadataToNewSession(session, newSession);
printf("StopCaptureNativeCallId\n\n");
		Stop(session);
		qos.Format("RtpNumPkts:%d RtpNumMissingPkts:%d RtpNumSeqGaps:%d RtpMaxSeqGap:%d", session->m_numRtpPackets, session->m_rtpNumMissingPkts, session->m_rtpNumSeqGaps, session->m_highestRtpSeqNumDelta);
