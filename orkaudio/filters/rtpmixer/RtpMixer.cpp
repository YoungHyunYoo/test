...
//printf("__DoStats start\n");
	...
	if(	abs(seqNumDelta) > 1000.0  &&
		abs(timestampDelta) > 160000.0) //__ not run	
	{
//printf("1 ");
	...
	{
//printf("2 ");
		if(seqNumDelta > (double)seqMaxGap) // __ run here
		{
//printf("3 ");
			seqMaxGap = (unsigned int)seqNumDelta;
		}
		if(seqNumDelta < 0.0) //__ not run
		{
//printf("4 ");
			seqNumOutOfOrder++;
		}
		if(seqNumDelta != 1.0 && details->m_sequenceNumber != 1) // __ not run
		{
//printf("5 ");
			seqNumMisses++;
		}
	}
//printf("__DoStarts end\n");
}


void RtpMixer::AudioChunkIn(AudioChunkRef& chunk)
{
//printf("Start AudioChunkIn\n");
//printf("\n\n____ Im rtpMixer AudioChunkIn\n\n");
	CStdString logMsg;
//printf("1 ");
	if(m_error)
	{
	...
	if(details->m_marker == MEDIA_CHUNK_EOS_MARKER) // __ run here last packet(call exit)
	{
//printf("2 ");
		if(m_numInputChannels) // __ not run
		{
//printf("3\n ");
...
//printf("3.2\n");
	...
	else if(chunk->GetNumSamples() == 0) //__ not run
	{
//printf("4 ");
	...
	if(chunk->GetNumBytes() > 100000) //__ not run
	{
//printf("5 ");
		m_error = true;
	...
	if(details->m_encoding != PcmAudio) // __ not run
	{
//printf("6 ");
	...
	if(m_numOutputChannels == 2) // __ not run
	{
//printf("7 ");
		CreateChannels(details->m_channel);
	}

	if(details->m_channel == 1) // __ run here when m_channel==1
	{
//printf("8 ");
		if(m_numInputChannels) // __ not run
		{
//printf("9 ");
		...
		if(m_lastChunkS1.get()) // __ run here
		{
//printf("10 ");
			...
			if( chunk->GetDetails()->m_timestamp - m_lastTimestampS1 == 20) // __ not run
			{
//printf("10.1 ");
		...
		else // __ run here when m_channel== 1 && first packet
		{
//printf("11 ");
			m_lastTimestampS1 = chunk->GetDetails()->m_timestamp;
		}
	...
	else if(details->m_channel == 2) // __ run here when m_channel ==2 
	{
//printf("12 ");
		if(m_numInputChannels) // __ not run
		{
...
//printf("14 ");
		...
		if(m_lastChunkS2.get()) // __ run here
		{
//printf("15 ");
			...
			// Correct timestamp of RTP packets sent by rogue User Agents who report it in miliseconds instead of in number of samples
			if( chunk->GetDetails()->m_timestamp - m_lastTimestampS2 == 20) // __ not run
			{
//printf("15.1 ");
		...
		else // __ run here when m_channel ==2 && first packet
		{
//printf("16 ");
			m_lastTimestampS2 = chunk->GetDetails()->m_timestamp;
		}

		if(m_oneS1PacketState) // __ run here when m_channel ==2 && first packet
		{
//printf("17 ");
		...
		{
//printf("18 ");
			if(!m_timestampCorrectiveDelta && (m_writeTimestamp != 0)) // __ not run
			{
//printf("18.1 ");
				m_timestampCorrectiveDelta = (double)details->m_timestamp - (double)m_writeTimestamp;
			}
...
//printf("22 ");
		...
		{
//printf("23 ");
			if(m_invalidChannelReported == false) //__ not run
			{
	...
	if(m_log->isDebugEnabled()) // __not run
	{
//printf("24 ");
	...
	if(m_writeTimestamp == 0) // __ run here when first packet
	{
//printf("25 ");
		if(details->m_channel == 1) // __ run here when first packet
		{
//printf("26 ");
...
//printf("27 ");
	...
	else if (correctedTimestamp >= m_readTimestamp) // __ run here
	{
//printf("28 ");
		if( (int)(rtpEndTimestamp - m_writeTimestamp) <= (int)FreeSpace() && (int)(m_writeTimestamp - correctedTimestamp) <= (int)UsedSpace()) // __ run here
		{
//printf("29 ");
			// RTP packet fits into current buffer
			StoreRtpPacket(chunk, correctedTimestamp);

			if(UsedSpace() > m_numSampleTrigger) //__ run here
			{
//printf("__usedSpace : %d, numSampletrigger : %d\n",UsedSpace(),m_numSampleTrigger);
//printf("29.1 ");
...
//printf("31 ");
	...
	if(m_log->isDebugEnabled()) // __ not run
	{
//printf("32 ");
...
{
//printf("__ Im RtpMixer AudioChunkOut\n");
...
{
//printf("___StoreRtpPacket start : ");
	CStdString debug;
...
//printf("2 ");
	...
	if(endRtpTimestamp64 >= 4294967296ULL)	//2^32=4294967296 // __ not urn
	{
//printf("3 ");
...
//printf("4 ");
	...
	if (silenceSize >= NUM_SAMPLES_CIRCULAR_BUFFER) // __ not run
	{
printf("5 ");
		if(m_toLog == true) // __ not run
		{
printf("6 ");
	...
	else if(silenceSize > 0) // __ run here
	{
//printf("7 ");
...
//printf("7.1 ");
			...
			m_writePtr++;
			if(m_writePtr >= m_bufferEnd) // __ some time run here
			{
//printf("7.2 ");
				m_writePtr = m_buffer;
			}
...
//printf("9.3 ");
		...
		tempWritePtr++;
		if(tempWritePtr >= m_bufferEnd) // __ some time run here
		{
//printf("9.4 ");
...
{
//printf("___ CreateShipment start : ");
