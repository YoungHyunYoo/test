...
#else
	if(pluginsDirectory.size() == 0) // __ run
	{ 
		// default unix plugins directory
		pluginsDirectory = "/usr/lib/orkaudio/plugins/"; // __ there is librtpmixer, libsilkcodec
	}
...
void MainThread()
{
	...
	if (!ACE_Thread_Manager::instance()->spawn(ACE_THR_FUNC(SttProcessing::ThreadHandler)))
	{
		LOG4CXX_INFO(LOG.rootLog, CStdString("Failed to create SpeechToText processing thread"));
	}
...
//	}
	...
	if(capturePluginOk)
	{
//printf("__ capturePluginProxy::Run()\n");
